...
function Get-NodeSqlConnectNameList {
    # Prefer FQDN, then discovery/short name — some hosts only have SPNs on one form.
    param([object]$Node)
...
function Wait-SqlEngineServiceRunning {
    # Node readiness without Kerberos: ask the host if SQL Engine is Running (same path as Restart-DbaService).
    param(
        [string]$Computer,
        [PSCredential]$Credential,
        [int]$TimeoutSeconds = 180
...
function Wait-SqlInstanceReady {
    <#
      1) Confirm Engine is Running on the node (WinRM/CIM — no SSPI).
      2) Probe SQL login briefly. Prefer -SqlCredential; Windows auth SSPI fails fast (does not burn 180s).
      Returns the SqlInstance name that accepted a login.
    #>
    param(
        [string]$Computer,
        [string[]]$SqlInstance,
    ...
    # Optional TCP ping to default port (named instances may use another port — ignore failures).
    try {
